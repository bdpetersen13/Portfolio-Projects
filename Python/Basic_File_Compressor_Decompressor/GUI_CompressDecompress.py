...
def openFile():
    fileName = filedialog.askopenfilename(
        initialdir='/', title="Please Select A File to Compress")
...
compressButton = tkinter.Button(
    window,
    text="Compress File",
    command=lambda: compression(compressInputEntry.get(),
                                compressOutputEntry.get()))
...
compressSearchButton = tkinter.Button(
    window,
    text="Search",
    command=lambda: compression(openFile(), "compressedoutputfile.txt"))
...
#Creating widget for compressing selected files by using a lambda function to get the real time values of input and output field
decompressButton = tkinter.Button(
    window,
    text="Decompress File",
    command=lambda: decompression(decompressInputEntry.get(),
                                  decompressOutputEntry.get()))

#Creating a Search widget to search for file to decompress if application is not already in the needed file directory
decompressSearchButton = tkinter.Button(
    window,
    text="Search",
    command=lambda: decompression(openFile(), "decompressOutputFile.txt"))
