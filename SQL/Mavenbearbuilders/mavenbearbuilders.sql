...
-- Data was incorrectly added to the refund CSV when they were actually customer inquiries
...
-- Verifyiong new items were added to the Products table
SELECT * FROM products;
...
-- The table will be back-populated using teh records from the order_items table
CREATE TABLE orders (
	order_id BIGINT,
    created_at DATETIME,
    website_session_id BIGINT,
...
-- Adding additional website data to be tracked to optomize the business performance. Only keeping the most recent month of data stored
...
-- Emptying the website_pageviews table. Data is only relevant for a couple months
